...
struct Processor
{
		...
		inputFile(parameters["input"].as<std::string>(), std::ios::binary)
	{
        ...
        printf("wavdatasize:%d, framecnt:%d, readcnt:%d\n", wavData.size(), inputFrameCount, inputFile.gcount());
		inputFramesPad = 1 << 12;
			...
			for (int i = 0; i < inputFrameCount; ++i)
				for (int c = 0; c < channelCount; ++c)
                {
                    inputBuffer[c * inputChannelStride + inputFramesPad + i] = toFloat(read<int16_t>(&wavData[(i * channelCount + c) * sizeof(int16_t)]));
                     //printf("wav:%f, input:%d\n", wavData[(i * channelCount + c) * sizeof(int16_t)], inputBuffer[c * inputChannelStride + inputFramesPad + i]);
        ...
        std::cout<<"wav new size"<<wavData.size()<<std::endl;
            for (size_t i = 0; i < 100; i++) {
        std::cout << inputBuffer[i + inputFramesPad] << " ";
    }
		restart(request);
	}
	...
	const float *getInputAudio(InputChunk inputChunk) const
	{
		...
		if (inputChunk.begin != inputChunk.end)
		{
            ...
            printf(" %f", inputBuffer[inputChunk.begin]);
            //std::cout<<"chun bgin:"<<inputChunk.begin<<std::endl;
		}
		return audio;
	}
	...
	bool writeSamples(Bungee::OutputChunk chunk)
	{
        ...
        //printf("outputsize:%d, counta;%d, chncnt:%d, oend:%d\n", wavData.size(), count, channelCount, wavData.end() - o);
